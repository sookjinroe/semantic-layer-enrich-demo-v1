...
매핑이 스코프 안팎에 걸치면: 찾은 컬럼을 전부 mappings 에 담고, 안 컬럼이 하나라도 있으면 mapped, 전부 밖이면 out_of_scope 다.
mappings 의 table 은 유추하지 마라 — 엔티티의 @Table(name=...) 선언에서 확인한 실제 이름을 쓴다 (클래스명 LoanProduct 의 테이블은 m_loan_product 가 아니라 m_product_loan 일 수 있다).
