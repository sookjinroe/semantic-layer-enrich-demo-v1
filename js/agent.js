...
[Description = 밀도, 길이가 아니다]
- 소비자(NL)는 get_column 으로 이름·타입·nullable·pk·fk 를 *이미* 본다. description 의 일 = 거기서 못 얻는 사실을 *빠짐없이*, 군더더기 없이 담는 것. 길이는 목표가 아니라 결과다.
- 담아라(해당하면): 값공간의 *의미*(라벨·단계), 형식, 분류(PII 등), 유사 컬럼과의 *구분*, 계보, 주의·gap. 이건 NL 이 못 얻는 것이므로 *짧게 만들려고 버리지 마라*.
- 빼라: 이미 아는 것(타입·null·"N자리 코드" 재서술), 행동 못 하는 것(분포·퍼센트 나열, 일반 SQL 코칭), 출처(→ evidence 에만).
...
answer: {"action":"answer","description":"NL이 get_column 으로 못 얻는 사실 전부, 군더더기 없이","confidence":"HIGH|MEDIUM|LOW","evidence":["쓴 신호+출처 예: orm, profile, code:LoanApplicationService.java"],"conflicts":[{"type":"…","detail":"…"}],"route_to_human":{"needed":false,"reason":""},"thinking":"1문장"}
