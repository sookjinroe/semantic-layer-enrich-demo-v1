...
//   balanced : 구조·간결함은 유지하고 암호 토큰만 풀어쓴 절충본 (기본)
//   verbose  : 문장으로 완전히 풀어쓴 서술본
//   셋의 규칙·구조·의도·출력 스키마는 동일. 표현(작성 방식)만 다르다.
//
// v3 (구조화 슬롯) 시스템 프롬프트는 별도 파일 js/prompt-v3.js 에 있다.
// v2 / v3 는 뷰가 달라 탭으로 분리되어 있고, 프롬프트 셀렉터는 v2 탭에서만 의미가 있다.
// ============================================================
...
같은 op 를 같은 인자로 반복하지 마라. [남은 횟수] 가 0 이면 반드시 answer 를 내라.`;
  ...
  const LIST = [
    { id: "compact",  label: "압축형(compact)",  system: COMPACT },
    { id: "balanced", label: "절충형(balanced)", system: BALANCED },
    { id: "verbose",  label: "서술형(verbose)",  system: VERBOSE },
  ];
  function get(id) { return LIST.find((p) => p.id === id) || LIST[0]; }
  function getSelectedId() {
    const s = localStorage.getItem("render_prompt");
    return LIST.some((p) => p.id === s) ? s : "balanced";
  }
